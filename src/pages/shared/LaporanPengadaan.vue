  ...
  <DefaultLayout>
    <div class="p-6">
        ...
        <div class="bg-white rounded-lg shadow">
          ...
          <div class="p-6">
             <div v-if="activeTab === 'barang'">
               <div class="mb-4">
                 <input v-model="searchBarang" type="text" placeholder="Cari nama barang..." class="border rounded px-3 py-2 w-64"/>
               </div>
                  ...
                  <table class="w-full table-auto border-collapse border">
                    ...
                    <tbody>
                      <tr v-for="pengajuan in laporanPengajuan" :key="pengajuan.id_pengajuan" class="border-b hover:bg-gray-50">
                        ...
                        <td class="border px-4 py-3 text-center">
                          <span :class="getStatusClass(pengajuan.status_pengajuan)" class="px-2 py-1 rounded-full text-xs font-medium">{{ pengajuan.status_pengajuan }}</span>
                        </td>
      ...
      <div class="mt-6 flex justify-end">
        <button @click="exportLaporan" class="bg-green-600 text-white px-6 py-2 rounded hover:bg-green-700 flex items-center gap-2" :disabled="loading">
          📊 Export Excel
        </button>
      </div>
    </div>
...
import { useLaporanStore } from '@/stores/laporanStore'; // ✅ Import the new store
...
// --- COMPUTED ---
// This simple filtering is acceptable to keep in the component
...
// --- FORMATTERS (Purely presentational, fine to keep here) ---
...
const getStockStatus = (current, min) => (current === 0) ? 'Habis' : (current <= (min || 5)) ? 'Rendah' : 'Normal';
const getStockStatusClass = (current, min) => (current === 0) ? 'bg-red-100 text-red-800' : (current <= (min || 5)) ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800';
</script>